...
func (d *ChainDispatcher) VerifySignedTransaction(ctx context.Context, request *account.VerifySignedTxRequest) (*account.VerifySignedTxResponse, error) {
	resp := d.preHandler(request)
	if resp != nil {
		return &account.VerifySignedTxResponse{
			Code: common.ReturnCode_ERROR,
